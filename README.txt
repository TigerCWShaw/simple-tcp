...
2. Data transmission and reception
    After establshing connection with the reciever, sender will send file1.txt in multiple packets each <= maximum segment size. (sender.py line 133) I set the max segment size to 16 byte for more convenient testing.
    
    The reciever will save the packets in a dictionary, when it recieves the fin request, it will combine the packets and save it in file2.txt
